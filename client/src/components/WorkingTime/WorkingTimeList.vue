  ...
  <b-card no-body>
    <b-row align-v="center" slot="header">
      <b-col cols="8" xl="12" md="12">
        <h1 class="mb-0">Working Times List</h1>
      </b-col>
    </b-row>
    ...
    <b-row class="mt-3 mr-3 mb-3 ml-3" v-if="edit">
      <working-time-create
        v-on:event_child="workingTimeCallback"
        :userId="userId"
      />
    </b-row>
...
export default {
  ...
  data() {
    return {
      filters: {
        date: {
          value: {
            end: moment(moment().add("days", 7).format("YYYY-MM-DD")).format(
              "YYYY-MM-DDTHH:mm:ss"
            ),
            start: moment(moment().format("YYYY-MM-DD")).format(
              "YYYY-MM-DDTHH:mm:ss"
            ),
          },
  ...
  methods: {
    ...
    refreshWorkingTimes: async function () {
      this.items = await getWorkingTimesBetweenDates(
        this.userId,
        this.filters.date.value.start,
        this.filters.date.value.end
      );
    },
    ...
    dateFilter(filterValue, row) {
      return (
        (row.start >= filterValue.start && row.start <= filterValue.end) ||
